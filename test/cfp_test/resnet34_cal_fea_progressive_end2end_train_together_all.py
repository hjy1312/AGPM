...
from dataset_cfp import ImageList
...
parser.add_argument('--gallery_list', default='./aligned_Pair_list_F.txt', type=str, metavar='PATH',
...
for model_ii in model_index_set:
    ...
    for list_name in ['./aligned_Pair_list_F.txt','./aligned_Pair_list_P.txt']:
    #for list_name in ['./aligned_Pair_list_P.txt']:
        opt.gallery_list = list_name
        F_True = 'list_F' in list_name
        ...
        for i, (real_cpu) in enumerate(dataloader_gallery):
            print('processing %d th batch' %(i+1))
            #print(data)
            batch_size = real_cpu.size(0)
            if F_True:
                w1 = torch.FloatTensor([0.0]*batch_size).unsqueeze(1)
                w2 = torch.FloatTensor([0.0]*batch_size).unsqueeze(1)
                w3 = torch.FloatTensor([0.0]*batch_size).unsqueeze(1)
            else:
                w1 = torch.FloatTensor([1.0]*batch_size).unsqueeze(1)
                w2 = torch.FloatTensor([1.0]*batch_size).unsqueeze(1)
                w3 = torch.FloatTensor([1.0]*batch_size).unsqueeze(1)
            if opt.cuda:
                w1 = w1.cuda()
                w2 = w2.cuda()
                w3 = w3.cuda()
    ...
    root_dir = './cfp_protocol/Protocol/Split/'
    #f_out = open('./cos_similarity_w.txt','w')
    save_dir = './fea_cfp/' + 'model_' + str(model_ii+1)
...
save_dir = osp.join(path_prefix,'auc_eer_tpr')
if not osp.exists(save_dir):
    os.makedirs(save_dir)
src_path = './std_show.py'
dst_path = osp.join(save_dir,'std_show.py')
shutil.copyfile(src_path,dst_path)
src_path = './show_eer.py'
dst_path = osp.join(save_dir,'show_eer.py')
shutil.copyfile(src_path,dst_path)
...
save_txt = osp.join(path_prefix,'acc_cfp.txt')
np.savetxt(save_txt,acc_set,fmt=['%s']*acc_set.shape[1],newline = '\n')
save_txt = osp.join(path_prefix,'std_cfp.txt')
